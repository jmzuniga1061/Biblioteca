...
async function ensureRoles() {
    ...
    for (const catName of categoriesList) {
        const existingCat = await exports.prisma.category.findUnique({ where: { name: catName } });
        if (!existingCat) {
    ...
    for (const b of booksToSeed) {
        let author = await exports.prisma.author.findFirst({ where: { name: b.author } });
        if (!author) {
            author = await exports.prisma.author.create({
                data: {
                    name: b.author,
        ...
        const category = await exports.prisma.category.findUnique({ where: { name: b.category } });
        if (!category)
            continue;
        const existingBook = await exports.prisma.book.findUnique({ where: { isbn: b.isbn } });
        const randomPrice = parseFloat((Math.random() * (50 - 5) + 5).toFixed(2));
        if (!existingBook) {
            await exports.prisma.book.create({
                data: {
                    title: b.title,
                    isbn: b.isbn,
            ...
            await exports.prisma.book.update({
