...
class ReportsService {
    async loansMonthly() {
        const result = await prisma_1.default.$queryRawUnsafe(`SELECT TO_CHAR("loanDate", 'YYYY-MM') AS month, COUNT(*) AS count
       FROM "Loan"
       GROUP BY month
       ORDER BY month DESC`);
    ...
    async booksTop(limit = 10) {
        const result = await prisma_1.default.$queryRawUnsafe(`SELECT b.id AS "bookId", b.title, COUNT(l.id) AS "loanCount"
       FROM "Book" b
    ...
    async usersActive(limit = 10) {
        const result = await prisma_1.default.$queryRawUnsafe(`SELECT u.id AS "userId", u.name, u.email, r.name AS "roleName", COUNT(l.id) AS "loanCount"
       FROM "User" u
